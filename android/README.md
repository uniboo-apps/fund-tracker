...
`fund-tracker` の投信・個別株データを、Android の**本物のホーム画面ウィジェット**として表示するアプリ。
...
## 構成
- `FundWidgetProvider.kt` … 6 provider（サイズ×テーマ固定）。タップで `data.js` の全銘柄（投信・stocks.json の個別株）を巡回・サイズ算出・更新
